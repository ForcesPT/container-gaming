...
class SelkiesLaunchCommand(unittest.TestCase):
    ...
    def test_stock595_differential_image_also_applies_unix_transform(self):
        dockerfile = (ROOT/'Dockerfile.stock595').read_text()
        self.assertTrue('COPY scripts/dpad-patch-selkies-unix /opt/dpadcloud/dpad-patch-selkies-unix' in dockerfile,
                        'stock595 differential image does not copy Unix transport patcher')
        self.assertTrue('RUN python3 /opt/dpadcloud/dpad-patch-selkies-unix ' in dockerfile,
                        'stock595 differential image does not apply Unix transport patcher')
