...
int ioctl(int fd, unsigned long request, ...)
{
    ...
    case 0x13: /* JSIOCGNAME(len) get identifier string */
        ...
        size_t nlen = strlen(interposer->js_config.name);
        strncpy(name, interposer->js_config.name, nlen);
        name[nlen] = '\0';

        va_end(args);
        return nlen; /* JSIOCGNAME returns the name length (SDL3 IsJoystick treats <=0 as failure -> rejects the device) */
