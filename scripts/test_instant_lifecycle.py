...
class LifecycleTests(unittest.TestCase):
    ...
    def test_cleanup_is_exact_and_does_not_call_unsafe_slot_stop(self):
        ...
        engine.stop_session.assert_called_once_with(SESSION, 0)
        engine.remove.assert_not_called()
    ...
    def test_launch_and_cleanup_cli_share_the_slot_fence(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            launcher.write_text('#!'+sys.executable+'\nimport sys,os\nfrom pathlib import Path\nassert sys.argv[-1] == '+repr(SESSION)+'\nif sys.argv[1] == "launch":\n assert os.environ.get("DPAD_SESSION_BACKEND") == '+repr(str(helper.parent / 'dpad-launch-session'))+'\n Path("/run/engine").touch()\nelse: Path("/run/engine").unlink()\n');launcher.chmod(0o755)
    ...
    def test_root_cli_cleanup_in_isolated_namespace_and_unprivileged_refusal(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            docker.write_text('#!'+sys.executable+'\nimport sys\nsys.exit(0 if sys.argv[1] in ("ps","container") else 1)\n');docker.chmod(0o755)
            controller=Path(tmp)/'controller'
            controller.write_text('#!'+sys.executable+'\n'+(helper.parent/'dpad-session-control').read_text().split('\n',1)[1]); controller.chmod(0o755)
            cmd=['bwrap','--unshare-user','--uid','0','--gid','0','--tmpfs','/','--ro-bind','/usr','/usr','--ro-bind','/lib','/lib','--ro-bind','/lib64','/lib64','--ro-bind','/home','/home','--ro-bind','/etc','/etc','--proc','/proc','--dev','/dev','--dir','/run','--ro-bind',str(helper.parent),str(helper.parent),'--tmpfs','/usr/bin','--ro-bind',str(docker),'/usr/bin/docker','--tmpfs','/usr/local/bin','--ro-bind',str(controller),'/usr/local/bin/dpad-launch-session',sys.executable,'-I',str(helper),*args]
            result=subprocess.run(cmd,capture_output=True,text=True)
            self.assertEqual(result.returncode,0,result.stderr)
    ...
    def test_engine_adapter_checks_exit_and_parses_exact_id_inspection(self):
        ...
        engine.stop_session(SESSION, 0)
        self.assertEqual(calls[-1],['/usr/local/bin/dpad-launch-session','stop','0',SESSION])
