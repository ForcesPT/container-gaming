...
def main():
    ...
    with os.fdopen(fd, 'w') as lock:
        ...
        if config is not None:
            ...
            env = {**os.environ, 'DPAD_INSTANT_CONFIG': str(request), 'DPAD_WD_WIDTH': sys.argv[4], 'DPAD_WD_HEIGHT': sys.argv[5], 'DPAD_STREAM_FPS': sys.argv[6]}
            # An interrupted launch retains its request and pin for reconciliation.
            process = subprocess.Popen(['/usr/local/bin/dpad-launch-session', 'launch', slot_text, 'none', 'none', sys.argv[3], config['image']],
