...
>
> **Current validated state (as of §16.7, 2026-08-09; Scaleway Paris + UpCloud Helsinki added 2026-08-11):**
> - **2026-08-11: UpCloud (Helsinki) RE-VALIDATED on the wayland path** (container-gaming `8b39a27` + cloud `8b973fc`). The prior UpCloud validation (2026-07-30) was gamescope-headless. Two probes via `scripts/upcloud-ops.mjs` (provision a raw L4 from the worker) + the new `DPAD_SKIP_DRIVER_SWAP` knob: **(1) skip the swap, shipped 595 → the compositor crashes** `libEGL: pci id 10de:27b8, driver (null)` / `failed to create dri2 screen` / `Failed to create EGLDisplay: InitFailed(NotInitialized)` / `Panicked: RecvError` (the SAME failure as Scaleway's `-580-server` variant) → selkies crash-loops → **595 is unusable for the compositor (not just flicker) → the 595→580 downgrade is MANDATORY on UpCloud, not avoidable**; **(2) after the swap, open-580 580.178.04 → the compositor EGL-inits clean** (`GL Renderer: NVIDIA L4/PCIe/SSE2`, `OpenGL ES 3.2 NVIDIA 580.178.04`, `wayland-1` socket, `EGL hardware-acceleration enabled`) + sway + the dpad-launcher picker render + stream — **image + sound user-confirmed** (over HTTPS via a temp Caddy reverse-proxy route `upcloud-probe.dpadplay.com`; the direct `http://<ip>:16100` URL doesn't establish the WebRTC peer — secure-context gate, the `SESSION 1/3` loop). Swap ~4-5 min (faster than the old 6-10 min estimate). The remaining UpCloud cold-boot lever (a custom template with open-580 baked) is deferred — lower priority. The probe VM was torn down (0 orphans). See `cloud/docs/STATUS.md` (the 2026-08-11 UpCloud note at the top) for the full record. **With this, all 5 providers (Helsinki/Gravelines/Des Moines/Quebec/Paris) are re-validated on the `gst-wayland-display` compositor path.**
