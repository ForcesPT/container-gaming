...
| `/tmp/dpad_resolution` (in-container state file) | `DPAD_WD_WIDTH x DPAD_WD_HEIGHT` | **Live resolution switch (§18.7).** Written by the Selkies `_arg_res` data-channel handler (the in-stream **Resolution** dropdown). The entrypoint's `build_selkies_cmd()` + the Sway heredoc read it on every Selkies relaunch. NVENC cannot renegotiate dimensions in-place, so the DpadPlay-styled drawer explicitly tells the player to **manually refresh the page after selecting a resolution**; that refresh creates the new WebRTC/NVENC pipeline. Not an env var — a runtime state file. |
...
| Live Resolution dropdown missing / no `_arg_res` | The boot-time `patch_live_resolution.py` overlay adds the DpadPlay-styled stream drawer, the **Resolution** select (720p/1080p/1440p/4K), and the `_arg_res` handler. Picking a resolution writes `/tmp/dpad_resolution` and restarts Selkies. **Manually refresh the browser page afterward** to create a fresh WebRTC/NVENC pipeline at the new dimensions; the notice and Refresh button sit directly below the Resolution control. |
| Battle.net install stalls / `Agent.exe error=2` / `BLZBNTBTS0000005C` | RESOLVED by the umu-launcher pivot: `battlenet-launch` runs the Blizzard installer + Agent.exe under `umu-run` (GE-Proton inside the Steam Linux Runtime — Valve's tested wow64) instead of raw `wine`. Live-validated on an OVH L4: the 32-bit Agent.exe launches (no `error=2`), Battle.net installs + launches. Requires the `:dpad-SteamOS` rebuild that bakes `umu-launcher` 1.4.4. **First-launch speedup (build-time prebake, commit `31f38fa`):** the image bakes the winetricks-initialized prefix + the ~657 MB SLR + `Battle.net.config` + the downloaded `Battle.net-Setup.exe` at `/opt/dpadcloud/battlenet-prefix`; `battlenet-launch` copies it to the session prefix on first launch (~seconds) + runs ONLY the installer (the user clicks through it in the stream) — skipping the ~2-min `umu-run winetricks` + the SLR download. Falls back to the full runtime winetricks if the prebake is absent (older image) or the copy fails. |
...
container also fetches the current `patch_live_resolution.py` overlay, which
applies the DpadPlay visual system and manual-refresh guidance to Selkies.
