...
| Battle.net install stalls / `Agent.exe error=2` / `BLZBNTBTS0000005C` | The 2026-08-10 `battlenet-launch` rewrite runs the Blizzard installer + Agent.exe under `umu-run` (GE-Proton inside the Steam Linux Runtime container — Valve's tested wow64) instead of raw `wine` (Wine 11's experimental new wow64 strangles the 32-bit Agent). Requires the `:dpad-SteamOS` rebuild that bakes `umu-launcher` 1.4.4 (STORES-PLAN §18.4). The SLR + pressure-vessel download to `$HOME/.local/share/umu` on the first `umu-run` (~1–2 GB, one-time per VM). |
| `pressure-vessel-wrap: … bwrap: Can't mount proc on /newroot/proc: Operation not permitted` (umu/Proton launch) | The session container is missing the 4th bwrap-nest flag. The session already runs `--cap-add SYS_ADMIN` + `--security-opt seccomp=unconfined apparmor=unconfined` (the umu #156 / Noble-AppArmor-userns set); add `--security-opt systempaths=unconfined` to the `docker run` so bwrap can remount `/proc` in the new pid+mount namespace. `dpad-launch-session`/`vm-bootstrap` should forward it (a runtime-validation item on the first umu session). |
