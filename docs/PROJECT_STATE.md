...
>
> **The cuda>=13.3 no-stream bug + fix (deep).** On `cuda_max_good>=13.3` Blackwell hosts (driver 610.x, RTX 5080/5090) the Selkies stream never came up: the browser `SESSION`-looped with no SDP offer. Root cause was NOT the encoder (nvcudah264enc probed OK) and NOT TURN (coturn reachable) — it was the **Xorg GLX vendor**. nvidia 610.x ships only `libglxserver_nvidia.so` (a GLVND vendor module), NOT a standalone `libglx.so`; so the `libglx.so` Xorg loads is Mesa's (`libglx-mesa0`), which **self-registers its swrast/DRISWRAST GLX vendor for screen 0** BEFORE nvidia's vendor registers → `(II) GLX: Another vendor is already registered for screen 0` → nvidia rejected → GL falls back to Mesa/**zink** → `ximagesrc` can't capture the zink screen → no pipeline → no stream. (The `Another vendor` line is cosmetic on its own — nvidia can still win GL via direct DRI3 on healthy hosts — but on 13.3 the zink fallback broke capture.) On `cuda_max_good=13.0` hosts the nvidia DDX's DRI2 path won screen 0 first, so it worked.
>
> **Fix (commit ed5f9c2).** The Dockerfile bakes **xserver-xorg-core's GLVND-neutral `libglx.so`** (extracted from the `.deb` — `libglx-mesa0` replaces, not diverts, it) into `/usr/lib/xorg/modules/nvidia/extensions/libglx.so` (the nvidia private `ModulePath`, listed first by `xorg.conf`). The entrypoint then hides Mesa's `libglx.so` on the nvidia-DDX path (restored on the Xvfb fallback), so Xorg loads the neutral dispatcher, which loads only nvidia's vendor → nvidia wins screen 0 → GL on the GPU → `ximagesrc` captures → stream connects. Also dropped `-iglx` on the nvidia Xorg (indirect GLX off) and the renderer check now flags `zink`/`swrast` (not just `llvmpipe`). The fix is in the shared `base` stage + shared `entrypoint.sh`, so it applies to ALL targets (`:dpad-heroic`, `:dpad-heroic-rtx50`, `:dpad-SteamOS[-rtx50]`) — rebuild them all.
>
> **Validated live (RTX 5080 + RTX 5090, cuda 13.3, driver 610.43.02/03):** `nvidia GLX dispatcher present -> hiding Mesa's libglx.so`, `OpenGL renderer string: NVIDIA GeForce RTX 5080/PCIe/SSE2` (not zink), Selkies offer/answer exchanged, video plays. The earlier-round diagnostic blocks (GLX-layout periodic dump, GLVND-swap-finder) were removed once confirmed — the fix is just the Dockerfile bake + the entrypoint hide.

> **UPDATE 2026-07-10 — IMAGE SPLIT: one 16 GB image → two slim multi-stage images, `:dpad-heroic` (Vast Docker, ~3.9 GB) + `:dpad-SteamOS` (Vast VM, ~7 GB). Both build-validated.**
>
> **Why.** The two use-cases are near-complementary in their heavy components: Vast Docker (no userns) needs a Heroic desktop (XFCE + Firefox + Heroic) and does NOT need Steam/gamescope/Proton; the Vast KVM VM (userns) needs Steam/gamescope/Proton and does NOT need a desktop. A single 16 GB image carried both paths' weight on every pull. So the repo now builds **two images from one multi-stage Dockerfile**, sharing a `base` stage.
>
> **Architecture (one Dockerfile, `--target`):**
> - `interposer-builder` — compiles the Selkies joystick interposer `.so` (x86_64 + i386) + `libnvenc_fix.so`, so `gcc-multilib` never ships in the finals.
> - `base` (shared, 2.72 GB) — Selkies-GStreamer + coturn + cloudflared + `libnvenc_fix.so` + `cuda-cudart` + `cuda-nvrtc` + display/audio/Mesa/X/Python + VGL + the `dpad` user. No launcher, no desktop.
> - `vast-docker` → **`:dpad-heroic`** (~3.9 GB) — base + XFCE + Heroic + accountsservice + real Firefox (Mozilla apt repo, pinned over the Ubuntu snap stub). `DPAD_LAUNCHER=heroic` default. No Steam/gamescope/Proton.
> - `vast-vm` → **`:dpad-SteamOS`** (~7 GB) — base + Steam (pre-bootstrapped at build for fast-boot) + gamescope + PipeWire + `patch_selkies_pipewire` + `dpad_input_patch` + the Steam license `zenity` wrapper. `DPAD_GAMESCOPE=1` default. No desktop/Heroic/Firefox. **GE-Proton is NOT baked in** — native Steam downloads its own Proton at runtime. RTX 50/Blackwell → `:dpad-SteamOS-rtx50` (`--build-arg CUDA_VERSION=12.8.1 --build-arg CUDA_PKG=12-8`).
>
> **Base swap (the big win):** `nvidia/cuda:12.5.1-runtime-ubuntu24.04` → `nvidia/cuda:12.5.1-BASE-ubuntu24.04`. The runtime base's CUDA math libs (libcublas/libcusparse/libcufft/libnpp/libcusolver/libcurand/libnvjitlink, ~1.6 GB) + snapd (124 MB) are UNUSED by NVENC/Selkies — only `cuda-cudart` (cudaupload/cudaconvert) + `cuda-nvrtc` (nvh264enc JIT) are needed. `cuda-compat` (datacenter forward-compat) dropped too (consumer GPUs). Validated: base builds at 2.72 GB; Selkies `selkies_gstreamer` + `evdev` import OK (evdev's C ext built with `python3-dev build-essential libevdev-dev libudev-dev` installed then **PURGED** in the same RUN so the compiler doesn't ship).
>
> **Removed (both images — Selkies is the ONLY browser stream now):** moonlight-web-stream, Sunshine, Tailscale/native-Moonlight, steamcmd, dpad-launch, bubbleroot/proot, the GE-Proton pre-bake, the Proton-tools steamcmd download, the CUDA math libs, snapd, the build toolchain. Entrypoint: dropped the Sunshine / mws / mws-autopair / Tailscale launch blocks, made cloudflared Selkies-only, simplified the status banner + periodic dump + health loop. The gamescope path (`start_gamescope_session`) is unchanged (it already ran its own Selkies/coturn/cloudflared).
>
> **coturn/TURN fix (also landed 2026-07-10, pre-split):** auto-detect coturn's listen port from `VAST_(TCP|UDP)_PORT_<internal>` (was hardcoded `73478` — invalid >65535, coturn wrapped it to 7942 and it wasn't mapped → Selkies "Connection failed"), and **widened the dual-ICE `rtc_config.json` gate** to fire on ANY real public IP (was gated on `DPAD_PROVIDER=runpod`, so Vast Docker with `DPAD_PROVIDER` unset never built dual-ICE → the in-container peer tried to hairpin to its own public IP → zero `relay` ICE candidates → "Connection failed"). Both fixes are in `entrypoint.sh`. Vast Docker launch now just needs `-p 3478:3478` (do NOT use the old `73478` identity tag) — `DPAD_PROVIDER` is no longer required.
>
> **Build/push:** `docker build --target vast-docker -t forcespt/dpadcloud-gaming:dpad-heroic .` / `docker build --target vast-vm -t forcespt/dpadcloud-gaming:dpad-SteamOS .` (+ `--build-arg CUDA_VERSION=12.8.1 --build-arg CUDA_PKG=12-8` for `-rtx50`). `vm-bootstrap.sh` now points at `:dpad-SteamOS[-rtx50]`. Docs updated: `README.md` (two-image model), `VAST-VM-DEPLOY.md` (tag refs + §11 split table), `RUNPOD.md` (top note + Selkies launch).
>
> **Possible future ~1 GB VM trim (needs runtime test):** drop `steam-libs-amd64/i386` — the pre-baked `steamrt64/32` already include the Steam Runtime, so the system steam-libs may be redundant.
>
> **ADDENDUM 2026-07-10 (Vast Docker `:dpad-heroic` end-to-end VALIDATED):** launched on Vast Docker with `-p 3478:3478/udp -e SELKIES_TURN_PROTOCOL=udp` — Selkies connects, video+audio+input flow. **Validated working on RTX 4070 and RTX 3060 single-GPU hosts.** The browser gets `typ relay` ICE candidates (dual-ICE short-circuit over the in-container coturn) and the stream comes up. The full `:dpad-heroic` Docker options: `-p 22:22 -p 3478:3478/udp -e SELKIES_TURN_PROTOCOL=udp -e SELKIES_BASIC_AUTH_USER=dpad -e SELKIES_BASIC_AUTH_PASSWORD=pass --security-opt seccomp=unconfined --security-opt apparmor=unconfined --shm-size 2g --ulimit nproc=1048576:1048576 --ulimit nofile=1048576:1048576` (DPAD_LAUNCHER=heroic is the image default; DPAD_PROVIDER/SUNSHINE_PASSWORD are no-ops now).
>
> **Host-inbound gotcha (the real cause of the earlier "Connection failed"):** some Vast hosts do NOT forward inbound mapped ports to the internet. An RTX 3090 host (machine 54804) dropped BOTH the TCP and UDP coturn maps — a STUN/TURN Allocate (type 0x0003) UDP packet to `<publicIp>:<VAST_UDP_PORT_3478>` got NO reply from outside (while Google STUN from the same probe replied, proving the probe's UDP works). On such hosts the in-container Selkies peer still relays (it reaches coturn locally at 127.0.0.1:3478, so the container-side `typ relay` candidates appear) but the browser gets NO relay candidate (only `host`/mDNS) → "Connection failed". This is a host networking issue, NOT the image. Diagnosis: `py -3` sending a STUN Allocate to the mapped port — coturn must answer (401/Unauthorized). No reply ⇒ destroy + relaunch on a different host. The in-container coturn design assumes the host's mapped port is internet-reachable, which is true for most Vast hosts (RTX 4070/3060 validated) but not all. Future robustness option (not built): a `DPAD_EXTERNAL_TURN` env to point the rtc_config at a public/self-hosted reachable TURN so the path works on any host regardless of Vast inbound forwarding.

> **UPDATE 2026-07-09 — HEROIC GAMES LAUNCHER ON VAST DOCKER = WORKING (Epic + GOG + Amazon storefront; Proton-direct via a umu_run.py wrapper). The Vast Docker path was Steam-only-blocked (no userns -> pressure-vessel/CEF crash); Heroic is the storefront replacement and games now launch on Vast Docker.**
>
> **The problem this solves.** On Vast Docker there are no user namespaces (`unshare -U` EPERM even as root - the container runtime's seccomp/AppArmor, not relaxable from inside). Steam's UI is blocked there (webhelper forced into pressure-vessel -> CEF crashes; bubbleroot/proot too slow; `STEAM_RUNTIME=0` unsupported). The proven Vast Docker game path was headless `steamcmd + dpad-launch` (Proton-direct, single-player, no store UI). This adds a real **storefront + library + browser UI**: Heroic Games Launcher (Epic via Legendary, GOG via gogdl, Amazon via Nile - Ubisoft/EA are NOT native in Heroic, sideload only; use Lutris later for those).
>
> **Why Heroic works where Steam doesn't.** Heroic is Electron (NOT wrapped in pressure-vessel like Steam's CEF). It runs natively with `--no-sandbox` (Chromium's renderer sandbox uses userns; `--no-sandbox` disables it). Its store/login webviews render in-app. Validated live on Vast Docker (RTX 4070 / driver 580, and RTX 3060 / driver 610): Heroic UI appears in the Selkies browser stream, Epic store renders, OAuth login works, games install + launch.
>
> **The umu/pressure-vessel wall (the hard part) - and the fix.** Heroic launches Windows games via **umu-launcher**, which ALWAYS wraps them in a **pressure-vessel (bubblewrap) Steam Runtime container** that needs user namespaces - and umu **will not drop the container by design** (upstream issue #40, wontfix: "no intentions or plans on removing or disabling the runtime"). On Vast Docker the container either hard-crashes (real bwrap) or crawls via the bubbleroot/proot ptrace shim (protonfixes hangs; games render via DXVK then exit). The attempted fixes that did NOT work:
> - `export UMU_NO_RUNTIME=1` / `PROTONFIXES_DISABLE=1` in `heroic-launch` -> Heroic **strips them** from the game-launch env (the umu child's `/proc/.../environ` was empty for these). Confirmed via `tr '\0' '\n' < /proc/$(pgrep umu_run)/environ | grep`.
> - `UMU_NO_RUNTIME=1` set inside umu (via a wrapper) -> umu **1.4.1 ignores it** (log still shows `Running 'GE-Proton' using runtime 'steamrt4'` + the full bubbleroot container; `UMU_NO_RUNTIME` isn't even in `man umu`'s env list). `PROTONFIXES_DISABLE=1` IS honored though.
> - Heroic config `defaultSettings.disableUMU=true` (jq preseed in config.json) -> did **not propagate** to new games (new games still got `disableUMU=false`; Heroic hardcodes it per-game at install). The per-game `GamesConfig/<app>.json` `disableUMU=true` (the UI "Disable umu" toggle) DOES work, but only per-game manually.
>
> **THE FIX (commit 2026-07-09, `entrypoint.sh` `heroic_wrap_umu()`): wrap `~/.config/heroic/tools/runtimes/umu/umu_run.py` with a tiny Python shim that BYPASSES umu entirely and execs GE-Proton DIRECTLY** - the same Proton-direct path `dpad-launch` PATH B validated on Vast Docker (and equivalent to Heroic's "Disable umu" = ON, which was confirmed working). The shim:
> ```python
> import os, sys
> proton = os.path.join(os.environ["PROTONPATH"], "proton")
> os.execv(proton, [proton, "waitforexitandrun"] + sys.argv[1:])
> ```
> Heroic still calls `python3 umu_run.py <game.exe> [args]` (its `--wrapper umu_run.py`), so the wrapper is used on every Play with zero UI changes. Heroic has already set `PROTONPATH` / `WINEPREFIX` / `STEAM_COMPAT_*` in the env, so Proton runs the game directly - no umu, no pressure-vessel, no bwrap, no proot, no userns. A background watcher (`heroic_wrap_umu`, gated to `DPAD_LAUNCHER=heroic`) wraps `umu_run.py` as soon as Heroic downloads it (not present at boot - Heroic fetches it on first run), idempotent (skips if the `DPAD_UMU_WRAPPER` marker is present), and re-checks every 10s then 60s in case Heroic re-downloads/updates umu. Boot log prints `[*] Heroic: wrapped umu_run.py -> Proton-direct (bypass umu/pressure-vessel container; no userns needed)`.
>
> **VALIDATED live on Vast Docker (RTX 4070 / driver 580.126.9):** launch.log shows `info: DXVK v2.7.1` + `info: Found device: NVIDIA GeForce RTX 4070` and **no `bubbleroot:` / `Running '...' using runtime 'steamrt4'` lines** (the pressure-vessel container is gone). Game (20 Minutes Till Dawn, Epic) opens and runs. New games work with no per-game toggle.
>
> **The wiring (one image, opt-in - safe for the VM/gamescope path):**
> - `Dockerfile` step `4d`: install Heroic `.deb` (v2.22.0, `ARG HEROIC_VERSION`) + `accountsservice` (Heroic queries `org.freedesktop.Accounts` over D-Bus; without it Heroic degrades - Steam-Headless #210) + `curl`. Step `4e`: install **Firefox** from Mozilla's apt repo (`packages.mozilla.org` - Ubuntu's `firefox`/`chromium` are snap stubs that fail in Docker) + register as `x-www-browser` so `xdg-open`/Heroic external links work. Heroic's Epic/GOG/Amazon LOGIN is in-app (Electron webview) and does NOT need Firefox; Firefox is only for external "buy on store" / target=_blank links.
> - `entrypoint.sh`: new `DPAD_LAUNCHER` env (`steam` default for the VM/RunPod full-Steam path | `heroic` for Vast Docker | `none` for bare desktop; back-compat: `DPAD_AUTOSTART_STEAM=0` -> `none` when `DPAD_LAUNCHER` unset). When `heroic`, the autostart + direct-launch blocks launch `/opt/dpadcloud/heroic-launch` (`heroic --no-sandbox`, logs to `/tmp/heroic.log`, in the periodic dump) instead of Steam. `DPAD_GAMESCOPE=1` still `exit 0`s before this section, so the VM multi-tenant gamescope path is **bit-for-bit unchanged** - the only VM cost is image size (~+500 MB Heroic + ~80 MB Firefox).
> - `scripts/heroic-launch`: the `--no-sandbox` wrapper (the umu bypass is in the entrypoint, not here - Heroic strips env from the umu child, so it must be done by wrapping umu_run.py itself).
> - Audio: the DFP/Vast-Docker path's `pulseaudio` daemon is best-effort (noble can refuse it when `pipewire-pulse` is installed from the gamescope step). The entrypoint now **falls back to `pipewire + wireplumber + pipewire-pulse` + `pactl module-null-sink`** when `pulseaudio` is absent - unified with the gamescope audio path. Fixes the `pulseaudio: command not found` / no-audio bug on Vast Docker.
>
> **Vast Docker launch options (Heroic path):** same as the validated Vast Docker recipe + `-e DPAD_LAUNCHER=heroic` (drop `DPAD_GAMESCOPE`). The coturn TURN networking auto-resolves from Vast's `PUBLIC_IPADDR` + `VAST_TCP_PORT_73478` (or `VAST_UDP_PORT_*`), unchanged. Selkies is the working browser stream (mws/Sunshine parked on Vast Docker - `/dev/uinput` blocked; revisit later). Note: on driver **610** the bundled GStreamer 1.24.6 `nvcodec` doesn't register NVENC-14 encode elements -> Selkies falls back to software `x264enc` (mws/Sunshine FFmpeg `h264_nvenc` is unaffected, but mws is parked). On driver **580** the encoder registers fine.
>
> **NEXT / open:** (1) Gamepad in **Wine** games - Windows games read gamepads via Wine's hidraw/evdev, not the Linux `/dev/input/js*` the Selkies joystick interposer hooks; validate with a controller (ArchWiki `DisableHidraw=1` winebus registry tweak is the fallback). (2) Stale `/opt/nvidia-drivers` lib versions (logs show `580.126.09` even on other driver hosts - the `install-display-drivers` extraction). (3) Image-size build-arg gating of Heroic+Firefox for VM-only builds. (4) `vm-bootstrap.sh` pass-through of `DPAD_LAUNCHER`. (5) Lutris for native Ubisoft/EA library browsing (Phase 2 - Heroic covers Epic+GOG+Amazon now).

> **UPDATE 2026-07-08 (NIGHT) — 4-GPU N-on-N VALIDATED (4× RTX 5060 Ti); lowercase PCI-compare fix for GPU slots with hex letters.** The 2-GPU validation used GPU slots 07/09 (no hex letters); scaling to 4 GPUs surfaced a `setup_nvenc_fix` bug: nvidia-smi `pci.bus_id` emits UPPERCASE hex (e.g. GPU 2 at `00:0B.0`) while `/proc/driver/nvidia/gpus` lists it lowercase (`0000:00:0b.0`). The PCI-bus→Device-Minor string compare was case-sensitive, so any GPU whose slot hex contains a letter (B, D, F…) FAILED to match → `VISIBLE_BITMASK` stayed 0 → fell back to nvidia-smi index→minor (always 0 inside a 1-GPU container) → `NVENC_FIX_AVAILABLE=0x1` (wrong) → the flexgrip interposer kept the wrong GPU (minor 0, not mounted) → NVENC failed → Selkies exited (502). Reproduced on the 4× RTX 5060 Ti VM: dpad-0/dpad-1 (slots 07/09) encoded fine; dpad-2/dpad-3 (slots 0B/0D) got the wrong mask 0x1 → DOWN. FIX (commit ee46fe4, `entrypoint.sh`): lowercase both `key` and `pkey` with `${var,,}` before comparing (bash 4+, entrypoint is `#!/bin/bash`) — applied to both `setup_nvenc_fix()` (gamescope path) and the DFP inline copy. After the fix the PCI match resolves the correct minor for EVERY GPU → correct mask → interposer keeps the container's actual GPU. VALIDATED on 4× RTX 5060 Ti (compute_cap 12.0, driver 580.95.05, 46 cpu / 202 GB): 4 containers, masks `0x1/0x2/0x4/0x8`, all 4 encoders 0 errors, all 4 Selkies UP, each sees a distinct GPU (4 UUIDs), resource partitioning 11 cpu / 49650 MB per tenant, video/audio/keyboard/mouse/gamepad confirmed on all 4. The fix generalizes to any N-GPU host (any slot with hex letters).**

> **UPDATE 2026-07-08 (LATE) — N-on-N MULTI-TENANT VALIDATED on 2× RTX 5060 Ti (Blackwell); NVENC #1249 fix shipped; Blackwell `-rtx50` auto-select; CPU+RAM resource partitioning.**
>
> **N-on-N = WORKING.** On a 2× RTX 5060 Ti Vast VM (compute_cap 12.0, driver 580.95.05), `vm-bootstrap.sh` launches one CDI container per GPU (dpad-0 on GPU 0 / /dev/nvidia0, dpad-1 on GPU 1 / /dev/nvidia1), each with its own coturn UDP port (3478→ext, 3479→ext), its own cloudflared quick-tunnel URL, its own Steam + Selkies + gamepad. Both streams serve video/audio/keyboard/mouse/gamepad on a fresh connection (validated live in the browser). Per-GPU isolation confirmed: each container's `nvidia-smi -L` shows exactly 1 GPU, different UUIDs.
>
> **NVENC #1249 fix (the blocker for N-on-N).** On driver 570+ a gamescope container pinned to a NON-ZERO GPU minor (dpad-1 → /dev/nvidia1) could not open an NVENC encode session: NVENC's `NV0000_CTRL_CMD_GPU_GET_ATTACHED_IDS` (RM cmd 0x0201 via /dev/nvidiactl) returns ALL host GPUs; NVENC then peer-inits with the unmounted /dev/nvidia0 and bails with `NvEncOpenEncodeSessionEx ... error code 2` (NV_ENC_ERR_UNSUPPORTED_DEVICE). This is the well-known nvidia-container-toolkit **#1249 / #1209** + k8s-device-plugin **#1282** regression (driver 570-580; affects FFmpeg/GStreamer nvh264enc/OBS; only fixed in 610.x). It hit at plugin-register time AND at peer-connect (`build_video_pipeline`: nvh264enc is None → `set_property` on NoneType → Selkies exits → 502). The `libnvenc_fix.so` (flexgrip) LD_PRELOAD interposer is the documented fix, but it was (a) only wired into the DFP path (after the gamescope `exit 0`), and (b) its /proc gpuId→minor matching matched on BUS only (`gpuId>>8 & 0xFF`), which FAILS on single-bus multi-GPU hosts (every GPU on bus 0, differing by SLOT) — it matched every gpuId to the first proc entry (all → minor 0) and gave up ("not filtering"). FIX (commit 44e4953): (1) `scripts/nvenc_fix.c` matches the FULL PCI address — `slot = gpuId & 0xFF`, `bus = gpuId>>8 & 0xFF`, `domain = gpuId>>16` (verified: gpuId 0x11800 → 0001:18:00.0; gpuId 0x7 → 0000:00:07.0), with the bus-only / domain:bus heuristics as fallback; (2) `entrypoint.sh` extracts the flexgrip detection into `setup_nvenc_fix()` and calls it in the gamescope path (start_gamescope_session) before the LD_PRELOAD assembly, threading `libnvenc_fix.so` ahead of the joystick interposer so the Selkies nvh264enc inherits it (auto-enabled on driver 570..609 when host GPU count > mounted /dev/nvidiaX; `DPAD_NVENC_FIX=1|0|auto`). Validated live on dpad-1: interposer now logs `KEEPING gpuId 0x9 (minor 1)`, `REMOVING gpuId 0x7 (minor 0)`, `filtered: 2 → 1 GPUs`; encoder registers with 0 errors; video/audio/gamepad work. (The `NVENC_FIX_DEBUG=1` log shows a benign ~3/s NVENC monitoring poll — cosmetic, not a loop bug.)
>
> **Blackwell `-rtx50` auto-select.** `vm-bootstrap.sh` `image_tag_for_gpu()`: compute_cap major ≥ 12 (sm_120/sm_121, RTX 50 series) → `:SteamUbuntu24.04VM-rtx50` (CUDA 12.8.1 — the only CUDA that knows Blackwell); else `:SteamUbuntu24.04VM` (CUDA 12.5.1). `DPAD_IMAGE_TAG` overrides. (The regular CUDA-12.5.1 image can't drive Blackwell — the Selkies encoder + cudaconvert/cudaupload would fail.) The `-rtx50` image is built with `--build-arg CUDA_VERSION=12.8.1 --build-arg CUDA_PKG=12-8`; the Dockerfile was already parameterized.
>
> **CPU + RAM resource partitioning (commit eca1321).** `vm-bootstrap.sh` `compute_session_resources()`: `per_cpus = (nproc - DPAD_RESERVE_CPUS)/N`, `per_mem = (MemTotal - DPAD_RESERVE_MEM_MB)/N`, applied per container as `--cpus` (hard CPU quota; `DPAD_CPU_MODE=quota|pinned|shares`), `--memory` + `--memory-swap` (hard RAM cap, no swap) + `--memory-reservation` (90% soft floor), `--pids-limit`. The GPU is already 1:1 per container via CDI (no GPU sharing needed for the N-on-N model). Defaults: reserve 2 CPU + 4 GB for the host; floor warnings at <3 CPU / <6 GB per session. Overrides: `DPAD_CPUS_PER_SESSION` / `DPAD_MEM_PER_SESSION`. For a 23-cpu/101GB/2-GPU VM → each tenant gets 10 cpu + ~47.5 GB + 1 GPU. Validated: `docker inspect` shows `NanoCpus=1e10` (10 cpus), `Memory=47.46 GiB`, `PidsLimit=4096` on both containers.
>
> **Known minor quirks (not blocking):** (1) **Browser refresh occasionally shows "Waiting for stream"** — a Selkies 1.6.2 WebRTC reconnect race (the new peer's ICE/data-channel doesn't re-establish; each peer DOES get a fresh webrtcbin via start_pipeline, so it's not stale state). Self-heals on a second refresh; a fresh incognito tab always works. NOT the NVENC fix (the interposer doesn't touch networking). A restart-on-disconnect supervisor would make it 100% consistent (deferred — adds a 5-8s reconnect flash; the quirk is intermittent + self-healing). (2) **NVRTC "invalid value for --gpu-architecture" errors** in selkies.log on Blackwell — the bundled GStreamer ships a CUDA 11.4 `libnvrtc` (`/opt/gstreamer/.../libnvrtc.so.11.4.152`) that can't JIT-compile for sm_120; the plugin falls back to a pre-compiled cubin so video works. Non-fatal/cosmetic (a real fix would make the CUDA 12.8 libnvrtc win, but the plugin links soname 11 — deferred). (3) `DPAD_LAUNCH_STAGGER` (vm-bootstrap, default 0) was added to space container launches but did NOT prevent the NVENC failure (it's not an encoder-vs-encoder race) — kept as an opt-in lever; the flexgrip fix is the real fix.
>
> **NEXT (open):** the reconnect-on-refresh quirk (restart-on-disconnect supervisor) if 100%-consistent refresh is wanted; oversubscription (more containers than GPUs) needs `keylase/nvidia-patch` or MIG/MPS for the consumer NVENC session cap; the NVRTC soname-11 fallback.

> **UPDATE 2026-07-08 COLD-BOOT 172s → 50s — killed the blanket `chown -R /home/dpad` that was 69% of boot. Root cause (found via timestamped `docker logs -t` + overlay upper-layer inspection): the entrypoint ran `chown -R dpad:dpad /home/dpad` on every boot to fix the handful of root-owned files a root boot process (D-Bus / install-display-drivers) creates in ~dpad (which would EPERM Steam's `mkdir ~/.local/share/icons`). BUT the build-time Steam pre-bootstrap (`scripts/build-bootstrap-steam.sh`) already owns ALL of ~dpad as `dpad:dpad` (uid 1001) — so 0 files actually needed chown. The blanket `chown -R` nonetheless called `chown()` on every one of the ~33,480 files in ~dpad (the pre-baked ~300 MB Steam client + Proton-GE). On the containerd overlayfs storage driver (`metacopy` NOT enabled in the mount options), every `chown()` — EVEN WHEN THE OWNER IS ALREADY CORRECT — triggers a full-data copy-up of the file from the lower (image) layer to the upper (container) layer. Result on a fresh `docker run`: ~33,480 files / ~4.4 GB copied up, ~119 s, for zero benefit. Proof: `find <upperdir>/home/dpad -type f | wc -l` = 33,480 before the fix; 0 (by the chown) after. Measured wall-time: `chown -R /home/dpad` = 119 s vs the targeted form 0.36 s. FIX (commit 2026-07-08, `entrypoint.sh`): replaced ALL 9 `chown -R <dir> dpad:dpad` calls with `find <dir> ! \( -user dpad -group dpad \) -exec chown dpad:dpad {} + 2>/dev/null || true` — chowns ONLY files not already `dpad:dpad` (preserves the EPERM fix: a synthetic root-owned file IS still caught, validated), skips the 33,480 already-correct files → no copy-up. Validated end-to-end on the Vast RTX 3060 VM (fresh `docker run` of the rebuilt `:SteamUbuntu24.04VM`): the gap between `audio sinks` and `Launching gamescope` dropped 119 s → ~0 s; total cold boot to the stream URL dropped **172 s → ~50 s** (-71%); video/audio/keyboard/mouse/gamepad all still work. The upper layer now holds only Steam's own runtime writes (~16k files / ~573 MB created during the 12 s Steam startup), NOT a blanket chown — expected + unavoidable.**
>
> **New cold-boot budget (fresh `docker run`, RTX 3060, 2026-07-08): NVIDIA driver `.run` download+install ~14 s (re-downloads ~80 MB every boot — host driver version unknown at build time) · gamescope+Steam → GAMESCOPE SESSION READY ~12 s (inherent Steam/pressure-vessel startup) · cloudflared quick tunnel ~10 s · Selkies start ~6 s · coturn ~3 s · targeted chown + gamepad setup ~1 s · CUDA+D-Bus+PipeWire+pipewire-pulse ~4 s = **~50 s total**. NEXT boot-time levers (deferred, in priority order): (1) cache the NVIDIA `.run` per driver-version in a host volume / `/var/cache/dpad-nvidia` so a re-boot of the same VM doesn't re-download (−14 s) — first boot of a given driver still pays; (2) parallelize cloudflared + Selkies startup with Steam's 12 s startup (overlap the ~16 s of cloudflared+Selkies with Steam) — needs care (Selkies must bind after the gamescope Xwayland :0 is up for `DPAD_INPUT_DISPLAY` discovery; cloudflared can start as soon as Selkies binds 16100); (3) the ~12 s Steam startup is largely inherent. The blanket-`chown -R` anti-pattern is now documented in VAST-VM-DEPLOY.md §10 — DO NOT reintroduce `chown -R` over `${USER_HOME}` or any large pre-baked dir; use the targeted `find ! \( -user X -group X \)` form.**

> **UPDATE 2026-07-07 (LATE) — three follow-ups from the live validation: (1) GAMEPAD STARTUP RACE FIXED in the interposer: on a fresh boot SDL3's inotify hotplug fires the instant the watcher mknods /dev/input/jsN, and if the Selkies gamepad socket isn't fully listening yet the interposer's first connect() fails — and the v1.6.2 retry loop re-called connect() on the SAME fd, which on Linux NEVER succeeds after a failed connect (the fd is left in error state; you must close()+socket() a fresh fd). So a lost race stuck the gamepad rejected for the whole session (it won the race on some boots, lost on others → flaky per-boot). Fix in scripts/joystick_interposer_v162.c: close()+socket() a fresh fd each attempt, timeout bumped 250→500ms → a lost race now self-heals within ~ms. Rebuilt .so (both arches) next image. (2) FLICKER on Steam menu open/close is an NVIDIA 580 driver regression (gamescope issue #1964, open), NOT our pipeline. The gamescope-steam.log `pipewire: out of buffers` + `xwm: got the same buffer committed twice, ignoring` on transitions are SYMPTOMS, not the cause: a GStreamer `queue` inserted after pipewiresrc (live-tested, leaky=upstream, max-size-buffers=16) did NOT reduce `out of buffers` OR the flicker (the hold is upstream of the queue), and `-r 60` (explicit gamescope framerate) did nothing (reverted) — confirming it is not a frame-rate/pool/always-copy issue we can fix. gamescope #1964 reports the exact symptom (Steam overlay/notifications/menu transitions cause artifacts+flicker) on NVIDIA 580 + gamescope, fixed by downgrading the NVIDIA driver 580 → 575 (multiple users + OP confirmed; gamescope flags `--immediate-flips`/`--force-composition` do NOT help). Our RTX 3060 is on driver 580.95.05 → matches. The fix is a HOST driver downgrade (575), which on a Vast VM we do not control (it's the host's driver). Decision: ACCEPT — documented as a known NVIDIA-580 limitation (mild, NVIDIA's bug, will be fixed upstream eventually); no image-side fix exists. (Mitigation for production: an offer filter preferring driver <580 avoids the regression, at the cost of a smaller host pool — deferred.) (3) NETWORK / "no internet" icon is Steam-side, NOT our image and NOT a NAT timeout: the Steam connection_log shows `ConnectionDisconnected('Disconnected By Remote Host') : 'Failure'` + `RecvMsgClientLogOnResponse: 'Try another CM'` every ~70s — Steam's CM servers bounce the WebSocket session (confirmed NOT a dual-login: the user is not logged in elsewhere; conntrack established timeout is 432000s and the drops are every ~70s, so not NAT). It's Steam's datacenter-IP handling (the Vast datacenter IP being bounced by CM load-balancing). Steam recovers + login/install/play work, but the icon + the ~70s bounce persist. A second, cosmetic Steam-UI error `TypeError: SteamClient.System.Network?.StartScanningForNetworks is not a function` (library.js) is a Steam client/JS-API version mismatch (Steam's own code), pre-existing, not fixable from the image.**

> **UPDATE 2026-07-07 CURSOR FIX (gamescope path) — the missing mouse cursor was TWO compounding issues: (1) gamescope `--backend headless` does NOT composite the X cursor into its PipeWire output (confirmed: a big red XDefineCursor does NOT appear in a captured pipewiresrc frame, while the XFIXES query shows a real 16x15 left_ptr arrow at the pointer), so `--enable_cursors=false` (relying on gamescope's cursor being in the frame) shows NO cursor; (2) Selkies' fallback cursor source is the XFIXES CSS-cursor overlay (`--enable_cursors=true`), but the Selkies web client AUTO-ENGAGES pointer lock on fullscreen + click (`requestPointerLock` in input.js _onFullscreenChange + click handlers), and browsers HIDE the CSS cursor during pointer lock → the overlay cursor is invisible too. So in both modes the user saw no cursor. FIX (both, committed): flip the gamescope Selkies launch to `--enable_cursors=true` (the XFIXES overlay is safe now that the dpad_input_patch.py .pth fixes the `add_extension_event` crash that previously 502'd selkies — confirmed: `Found XFIXES version 4.0` + `starting cursor monitor` + no crash) AND inject a shim into the web client's `/opt/gst-web/input.js` (`scripts/patch_gst_web_cursors.sh`, idempotent, applied build-time in the Dockerfile + re-applied by the entrypoint) that gates `Element.prototype.requestPointerLock` behind `window.DPAD_POINTER_LOCK` (default false) → no auto pointer lock → the server-sent CSS cursor stays visible (even in fullscreen) + mouse is ABSOLUTE (Selkies sends `m` not `m2`), which is correct for Steam-UI/desktop navigation. For an FPS game that needs relative mouse + pointer lock, run `window.DPAD_POINTER_LOCK = true` in the browser console then click the video. Validated live on the Vast RTX 3060 VM: cursor visible in both windowed + fullscreen; video/audio/gamepad unaffected. (The flicker on Steam menu open/close is a separate, mild gamescope-headless frame-pacing artifact on heavy UI transitions — `vblank: Using timerfd` synthetic 60Hz; no clean lever, not worth chasing. The Steam "can't reach steam servers / no internet" icon is cosmetic: api/store/login/community all return 200/302/404, only the non-existent `cm1.steampowered.com` hostname fails, and login+install+play all work.)**

> **UPDATE 2026-07-07 GAMEPAD (STAGE 3b) VALIDATED END-TO-END WITH A REAL CONTROLLER on a Vast RTX 3060 VM — Steam Big Picture navigates with D-pad/sticks/A. The TRUE root cause (one layer BELOW the JSIOCGNAME fix) was that the Dockerfile's i386 interposer rebuild SILENTLY FAILED (gcc-multilib / libc6-dev-i386 missing → `gcc -m32` → `cannot find crti.o` / `-lgcc`), so the OLD unpatched deb i386 `.so` (JSIOCGNAME returns 0) shipped in the image while only x86_64 was actually rebuilt. Steam's MAIN binary is 32-bit (`ubuntu12_32/steam`) → loads the i386 interposer → SDL3 `IsJoystick` (`ioctl(JSIOCGNAME) <= 0`) rejected the device → Steam never saw a gamepad. The previous "validated IsGamepad=1" was DECEPTIVE: it only tested the x86_64 path (sdl3_guid_test compiled 64-bit against x86_64 libSDL3 + x86_64 interposer); Steam's gamepad enumeration runs in the 32-bit steam binary via the i386 libSDL3 + i386 interposer. FIX: install `gcc-multilib libc6-dev-i386` in the Dockerfile (apt step before the rebuild) + remove the `2>/dev/null || true` so the i386 build fails LOUD; rebuild BOTH `.so` from `scripts/joystick_interposer_v162.c`. Live-confirmed after the fix: the interposer log now shows the FULL SDL3 classic probe sequence `JSIOCGNAME → JSIOCGBUTTONS → JSIOCGAXES → JSIOCGBTNMAP → JSIOCGAXMAP` (previously only `JSIOCGNAME` then reject+close), the 32-bit sdl3_guid_test reports `count=1, GUID=0000d60653656c6b69657320436f6e00, IsGamepad=1`, `js` events flow live, and Big Picture navigates. Dockerfile edit committed; the published `:SteamUbuntu24.04VM` image is STALE (still the broken i386 .so) → **NEXT: rebuild + push so a fresh-boot works with zero manual steps.** The Dockerfile change: line ~264 apt gains `gcc-multilib libc6-dev-i386`; line ~288 drops `2>/dev/null || true` (now `&&`, fails the build if the i386 rebuild breaks).**
>
> **Diagnostic that cracked it (reproduce on any future "gamepad not detected" instance):**
> 1. `docker exec -u 0 dpad-0 ls -la /usr/lib/i386-linux-gnu/selkies_joystick_interposer.so /usr/lib/x86_64-linux-gnu/selkies_joystick_interposer.so` — if i386 is dated ~Aug 2024 (the deb) and x86_64 is dated the image-build day, the i386 rebuild failed (image STALE).
> 2. `docker exec -u 0 dpad-0 gcc -shared -fPIC -O2 -m32 -ldl -o /tmp/t.so /dev/null 2>&1 | head` — `cannot find crti.o` / `cannot find -lgcc` = gcc-multilib missing.
> 3. `docker exec -u 0 dpad-0 tail /tmp/selkies_js.log` — if you see `JSIOCGNAME` but NOT `JSIOCGBUTTONS/JSIOCGAXES/JSIOCGBTNMAP/JSIOCGAXMAP`, SDL3 accepted the open but REJECTED after the JSIOCGNAME probe (i386 interposer returned 0). With the fix, ALL FIVE ioctls appear.
> 4. The 32-bit decisive test: copy `scripts/sdl3_guid_test.c` in, `gcc -O2 -m32 -o sdl3_guid_test32 sdl3_guid_test.c -L/home/dpad/.steam/debian-installation/ubuntu12_32 -l:libSDL3.so.0 -ldl -Wl,-rpath,/home/dpad/.steam/debian-installation/ubuntu12_32`, run as `dpad` with `LD_PRELOAD=/usr/lib/i386-linux-gnu/selkies_joystick_interposer.so SDL_JOYSTICK_LINUX_CLASSIC=1 SDL_JOYSTICK_DISABLE_UDEV=1 SDL_JOYSTICK_DEVICE=/dev/input/js0 SDL_GAMECONTROLLERCONFIG=0000d606...` while a browser gamepad is connected (so `/tmp/selkies_js0.sock` exists). Expect `count=1, GUID=0000d60653656c6b69657320436f6e00, IsGamepad=1`. (The 64-bit test passing while the 32-bit fails = the tell.)
>
> **Why "no process holds /dev/input/js0 open" is a red herring:** the interposer swaps the `/dev/input/jsN` fd for the socket fd (it intercepts `open()` and returns a connection to `/tmp/selkies_jsN.sock`). So the reader holds the SOCKET open, not the device node — `grep /proc/*/fd for /dev/input/js0` is always empty by design. Check `ss -lxp | grep selkies_js0` for the peer, or just trust `on_message head=js` climbing in `/tmp/selkies.log` + the interposer's full ioctl sequence.
>
> The full gamepad fix chain is now 6 layers (the 5 prior + this i386 rebuild): (1) wiring into `start_gamescope_session`, (2) `SDL_JOYSTICK_LINUX_CLASSIC=1`+`SDL_JOYSTICK_DEVICE`, (3) root watcher mknods `/dev/input/jsN` on socket appear, (4) `SDL_JOYSTICK_DISABLE_UDEV=1` for ENUMERATION_FALLBACK hotplug, (5) the JSIOCGNAME-returns-name-length source patch, (6) **ACTUALLY rebuilding BOTH x86_64 AND i386 `.so` (gcc-multilib) — the layer that was silently missing.** All 6 now in the Dockerfile.
>
> **NEXT: rebuild + push `:SteamUbuntu24.04VM`** (Dockerfile now installs gcc-multilib + rebuilds both arches + fails loud), then the remaining headline payoff: the **2-GPU N-on-N multi-tenant test** (one CDI container per GPU in `DPAD_GAMESCOPE` mode → N independent streamed+interactive+gamepad Steam UIs on one VM, no modesetting contention). Gamepad path is all per-container LD_PRELOAD + per-container `/dev/input/jsN` → no shared state → N-on-N is unblocked.
>
> The full gamepad fix chain (5 layers, each a real on-instance bug, commits `9efa4bf` → `a3acaab` → `14bcfb7` → `ebddd3c` → `dbeaa03`):
>
> 1. **Wiring (`9efa4bf`)** — the gamescope path `exit 0`s at ~line 651 BEFORE the global interposer setup at ~1054, so `DPAD_GAMESCOPE` mode had an empty `/dev/input` + Steam had no interposer. Fix: set up the interposer inside `start_gamescope_session` + thread `LD_PRELOAD`/`SDL_JOYSTICK_*`/`SELKIES_INTERPOSER` explicitly through the gamescope+Steam + Selkies launches (`as_user`/`su` strips the parent env).
>
> 2. **Classic path (`9efa4bf`)** — Steam ships **SDL3** (`libSDL3.so.0`, Jun 2025), which defaults to the evdev/libudev backend and ignores `/dev/input/js*` that the v1.6.2 interposer hooks. `SDL_JOYSTICK_LINUX_CLASSIC=1` + `SDL_JOYSTICK_DEVICE=/dev/input/js0` re-route SDL3 to the classic joystick backend (`JSIOCG*` ioctls + 8-byte `js_event` = exactly what the v1.6.2 interposer + `gamepad.py` serve). Validated with `scripts/jsclassic.c`: the interposer answers `JSIOCGVERSION=0x20100`, `JSIOCGNAME`, `JSIOCGBUTTONS=11`, `JSIOCGAXES=8`, `JSIOCGBTNMAP`/`JSIOCGAXMAP` + serves 4/4 js_events.
>
> 3. **Hotplug (`a3acaab`)** — SDL3 enumerates joysticks ONCE at SDL_Init (Steam startup), BEFORE the browser gamepad connects (the user presses a button only after Steam UI is up). With `/dev/input/jsN` pre-created at boot but no Selkies socket yet, the interposer's `open()` fails + SDL3 doesn't re-scan. Fix: do NOT mknod at boot; a root watcher daemon mknods `/dev/input/jsN` the instant Selkies' `/tmp/selkies_jsN.sock` appears (Selkies creates it when the browser connects a controller), and removes it when the socket goes. The node creation fires SDL3's hotplug. (mknod needs root; Selkies runs as dpad, so the watcher is a root loop in the entrypoint.)
>
> 4. **Fallback enumeration (`14bcfb7`)** — `SDL_GetSandbox()` only detects containers via `/.flatpak-info`, `SNAP*` env, or `/run/host/container-manager` — NONE exist in a plain Vast Docker container → SDL3 sees `SDL_SANDBOX_NONE` and uses the **libudev** path (libudev loads but there's no udev daemon → zero devices + the inotify/scandir hotplug, which only runs in `ENUMERATION_FALLBACK`, is NOT active). So the watcher's mknod was ignored (confirmed live: js0+js1 mknod'd, 605 browser events flowed, but the interposer logged no new `open()`). Fix: `SDL_JOYSTICK_DISABLE_UDEV=1` forces `ENUMERATION_FALLBACK`, activating the inotify `IN_CREATE -> MaybeAddDevice` hotplug (for js* in classic mode) + the 3s `/dev/input` scandir-mtime fallback. After this fix the hotplug fired: SDL3 opened `/dev/input/js1`, the interposer connected to `/tmp/selkies_js1.sock`, read the config ("Selkies Controller", 11 btns/8 axes), answered `JSIOCGNAME`.
>
> 5. **Gamepad mapping + the ROOT CAUSE (`ebddd3c` + `dbeaa03`):** Steam Big Picture drives gamepadui via the **SDL_GameController (gamepad) API**, not the raw joystick API. The v1.6.2 interposer presents a raw joystick named "Selkies Controller" with NO vendor/product ID (its `js_config_t` has no vendor/product fields + it doesn't intercept `JSIOCGID`), so SDL3 can't auto-map it as a gamepad. (a) Provide `SDL_GAMECONTROLLERCONFIG` with a mapping for the Selkies GUID. SDL3's GUID for a zero-vendor classic js device = `bus(0)+crc16(name)+name-bytes` (`SDL_CreateJoystickGUID`, vendor=0 branch): `crc16("Selkies Controller")=0x06d6` → GUID `0000d60653656c6b69657320436f6e00` (computed in Python from SDL's crc16, `src/stdlib/SDL_crc16.c`, modbus poly 0xA001). The mapping matches `STANDARD_XPAD_CONFIG` in `gamepad.py`: btn 0-10 = A/B/X/Y/TL/TR/SELECT/START/MODE/THUMBL/THUMBR, axes 0-7 = X/Y/Z/RX/RY/RZ/HAT0X/HAT0Y; the dpad arrives as axes 6/7 which SDL3 classic treats as **hat 0**, so `dpup:h0.1,dpdown:h0.4,dpleft:h0.8,dpright:h0.2` (not `-a7/+a7`). (b) **BUT that mapping alone wasn't enough** — `IsGamepad` was still 0. A decisive SDL3 test (`scripts/sdl3_guid_test.c`, linked against Steam's `libSDL3.so.0`) showed `SDL_GetJoysticks: count=0` — SDL3's `IsJoystick` REJECTED the device. Root cause: the v1.6.2 interposer's `JSIOCGNAME` handler **returns 0** (`return 0; // 0 indicates success` in `joystick_interposer.c` line 339), but the real `JSIOCGNAME` ioctl returns the **name length**. SDL3's `IsJoystick` (`src/joystick/linux/SDL_sysjoystick.c` line 316) checks `ioctl(fd, JSIOCGNAME(...)) <= 0` and treats `<=0` as failure → falls into `GuessIsJoystick` (evdev `EVIOCGBIT` probes the interposer doesn't hook) → `goto error` → **device rejected** → Steam never adds the joystick → no gamepad. **Fix:** rebuild the interposer `.so` from the patched source `scripts/joystick_interposer_v162.c` (one-line change: `return strlen(interposer->js_config.name);`) over the deb-installed one in the Dockerfile (x86_64 + i386). **Validated:** with the patched interposer the SDL3 test reports `count=1`, `GUID=0000d60653656c6b69657320436f6e00` (matches the computed GUID), **`IsGamepad=1`** ✅.
>
> **Architecture finding (unchanged, the frame for the whole approach):** gamepads bypass gamescope entirely. `LibInputHandler.cpp` handles only POINTER/KEYBOARD/TOUCH (libinput has no gamepad class); the EIS/libei socket (`InputEmulation.cpp`) advertises only POINTER/POINTER_ABSOLUTE/KEYBOARD/BUTTON/SCROLL — no gamepad axis path. Apps read `/dev/input/*` directly. So the gamepad analog of the XTest keyboard/mouse path is the Selkies joystick interposer (LD_PRELOAD shim: intercepts `open("/dev/input/jsN")` → `/tmp/selkies_jsN.sock`).
>
> **NEXT: rebuild + push `:SteamUbuntu24.04VM` (Dockerfile now COPYs + rebuilds the patched interposer), fresh-boot, and test with a REAL controller in the browser** — open the stream URL, login, press a controller button (Selkies web client sends gamepad-connect → creates `/tmp/selkies_jsN.sock` → watcher mknods `/dev/input/jsN` → SDL3 fallback hotplug → interposer connects (now returns the name length so IsJoystick ACCEPTS it) → `SDL_GAMECONTROLLERCONFIG` maps it as a gamepad → Steam Big Picture detects "Selkies Controller" + you can navigate with D-pad/sticks + A + play a game). Live checks: `docker exec dpad-0 tail /tmp/selkies_js.log` ("Intercepted open call for /dev/input/jsN" + "Read config from socket"), `docker exec dpad-0 ls /tmp/selkies_js*.sock`, and `docker exec dpad-0 ls /dev/input/js*` (the watcher-created node). All layers validated standalone; only Steam-Big-Picture-navigation with a real controller remains to confirm.
>
> **If Steam still doesn't navigate:** the evdev fallback is fully researched + preserved under `scripts/gamepad-evdev-fallback/` (fake-libudev + main-branch evdev interposer + `dpad_gamepad_patch.py` + dual-serve input_event dispatch, modelled on `selkies-project/selkies` main `src/selkies/input_handler.py`; README there documents when/how). It presents a real Xbox 360 VID/PID (0x045e/0x028e) so SDL3's default DB auto-maps it — no GUID guessing — but needs the dual-serve patch (the one piece not yet written).
>
> **Multi-tenant (N-on-N) note:** the gamepad path is all per-container LD_PRELOAD + per-container `/dev/input/jsN` nodes — no shared state, no uinput/caps beyond what we have. One CDI container per GPU in `DPAD_GAMESCOPE` mode → N independent streamed+interactive+gamepad Steam UIs on one VM, no modesetting contention. The 2-GPU N-on-N test is the remaining headline payoff after gamepad is confirmed.
>
> **Validation/test scripts in `scripts/` (preserved for regression):** `jsclassic.c` (SDL3 classic probe validator — the v1.6.2 interposer handles the full sequence), `sdl3_guid_test.c` (the decisive SDL3 test using Steam's libSDL3.so.0 — prints the actual GUID + IsGamepad; compiles with `gcc -O2 -o sdl3_guid_test sdl3_guid_test.c -L<dir with libSDL3.so.0> -lSDL3 -ldl -Wl,-rpath,<dir>`; run as the dpad user with the interposer LD_PRELOAD + `SDL_JOYSTICK_LINUX_CLASSIC=1 SDL_JOYSTICK_DISABLE_UDEV=1 SDL_JOYSTICK_DEVICE=/dev/input/js1`), `joystick_interposer_v162.c` (the patched v1.6.2 source baked into the image), `jsclient.c`/`jsfeeder.py` (v1.6.2 plumbing), `evclient.c`/`jsfeeder_main.py`/`udev_enumerate_test.c`/`dpad_gamepad_patch.py` (evdev fallback validation), `test_uinput.py` (uinput probe — ruled out: no udev → no /dev/input/eventN node), `run_*.sh` + `check_gamepad_env.sh` (driver scripts; the ssh `bash -c` parens break easily so prefer copying a script in + `docker exec bash /tmp/x.sh`).

> **UPDATE 2026-07-07 GAMEPAD (STAGE 3b) — wired via the SDL3 classic joystick path + the v1.6.2 Selkies joystick interposer; every layer validated; rebuild + real-controller test pending.**
>
> Gamepad is the last input modality. The architecture finding that shaped the whole approach: **gamepads bypass gamescope entirely.** gamescope's `LibInputHandler.cpp` handles only POINTER/KEYBOARD/TOUCH (libinput has no gamepad class), and the EIS/libei socket (`InputEmulation.cpp`) advertises only `POINTER/POINTER_ABSOLUTE/KEYBOARD/BUTTON/SCROLL` (TOUCH is even commented out) — **no gamepad axis path through gamescope or libei.** Apps read `/dev/input/*` directly via evdev/joystick. So the gamepad analog of the XTest keyboard/mouse path is the **Selkies joystick interposer** — an LD_PRELOAD shim that intercepts `open("/dev/input/jsN")` in the app and redirects to `/tmp/selkies_jsN.sock` (the socket Selkies' `gamepad.py` serves when the browser connects a controller). DIRECT, in-process, no gamescope/libinput/libei.
>
> **Root wiring bug found + fixed:** the gamescope path `exit 0`s at line ~651 BEFORE the global interposer setup at ~1054, so in `DPAD_GAMESCOPE` mode `/dev/input` was empty and Steam had no interposer loaded (confirmed on the live `dpad-0`: `/dev/input` empty, no `selkies_joystick_interposer.so` in any steam/gamescope process). Fix: set up the interposer inside `start_gamescope_session` (`export SELKIES_INTERPOSER` + `mknod /dev/input/js{0-3}` char devices major 13 + `export LD_PRELOAD`) and thread `LD_PRELOAD`/`SDL_JOYSTICK_DEVICE`/`SDL_JOYSTICK_LINUX_CLASSIC`/`SELKIES_INTERPOSER` explicitly through the gamescope+Steam launch (both the initial + health-loop relaunch) and the Selkies launch (`as_user` = `su -s /bin/bash dpad -c` strips the parent env, so the env must be re-exported inside each `as_user` command string — same pattern the DFP path uses). Selkies launch also gains `--js_socket_path=/tmp` (explicit; default already `/tmp`).
>
> **The SDL3/evdev problem (Selkies issue #168):** Steam ships **SDL3** (`libSDL3.so.0`, Jun 2025) which discovers gamepads via **libudev + evdev** (`/dev/input/event*`) and ignores the legacy joystick API (`/dev/input/js*`) the v1.6.2 interposer hooks. The evdev-capable interposer (Selkies PR #173) is **unreleased** (only on `main`) and uses an **incompatible socket protocol** (new `js_config_t` 1360 bytes + a 1-byte arch-byte handshake + dual-serve: `js_event` on `selkies_jsN.sock` AND `input_event`+SYN on `selkies_event100N.sock`). uinput was ruled out: `UI_DEV_CREATE` succeeds but **no `/dev/input/eventN` node appears** (container has no udev), so SDL3 can't discover it either.
>
> **The simplification (the active path):** SDL3 has `SDL_HINT_JOYSTICK_LINUX_CLASSIC` (env `SDL_JOYSTICK_LINUX_CLASSIC=1`) that re-enables the **classic `/dev/input/js*` backend** — it uses `JSIOCG*` ioctls + reads 8-byte `js_event` structs (exactly what the v1.6.2 interposer + `gamepad.py` serve). And `SDL_JOYSTICK_DEVICE=/dev/input/js0` makes SDL3 `MaybeAddDevice()` directly (no udev enumeration; SDL3 also auto-disables udev in containers via `SDL_GetSandbox()`). So **two env vars** route Steam's SDL3 to `/dev/input/js0` → v1.6.2 interposer → Selkies — no fake-libudev, no new interposer, no protocol patch, no dual-serve.
>
> **Validated end-to-end with a C client (`scripts/jsclassic.c`) that mimics SDL3's classic probe sequence exactly:** the v1.6.2 interposer intercepts `open("/dev/input/js0")`, reads the 1348-byte v1.6.2 config, and responds to every probe SDL3 classic issues — `JSIOCGVERSION=0x20100`, `JSIOCGNAME="Selkies Controller"`, `JSIOCGBUTTONS=11`, `JSIOCGAXES=8`, `JSIOCGBTNMAP=[304,305,307,308,310]` (BTN_A/B/X/Y/TL), `JSIOCGAXMAP=[0,1,2,3]` (ABS_X/Y/Z/RX) — and serves 4/4 `js_event`s (button A press/release + axis motion). The v1.6.2 socket protocol matches our `gamepad.py` exactly.
>
> **The one unproven link (the rebuild + real-controller test will confirm):** Steam's bundled SDL3 actually honoring `SDL_HINT_JOYSTICK_LINUX_CLASSIC` (SDL3 source confirms it reads the hint at `LINUX_JoystickInit`; the probe sequence is validated). If Steam does NOT detect the controller via classic, the **evdev fallback** is fully researched + preserved.
>
> **Evdev fallback (NOT wired in; preserved under `scripts/gamepad-evdev-fallback/`):** the official Selkies container-gamepad solution — build `fake-libudev` (`addons/fake-udev`, LD_PRELOAD → lies to libudev so SDL3 discovers 4 virtual XBox pads at `/dev/input/js0-3` + `/dev/input/event1000-1003`, no udev needed; validated: a libudev enumeration test sees all 8 nodes with `ID_INPUT_JOYSTICK=1`) + the `main`-branch evdev interposer (intercepts open/ioctl/read on the fake paths → Selkies sockets; validated: intercepts `open("/dev/input/event1000")`, reads the 1360-byte config, serves `input_event`s) + `dpad_gamepad_patch.py` (`.pth` making v1.6.2 `SelkiesGamepad.__make_config` emit the 1360-byte new `js_config_t`; validated: 1360 bytes, vendor 0x045e) + `mknod` dummy nodes. The remaining piece to write if needed: the dual-serve event dispatch (`input_event`+SYN on the `selkies_event100N` socket), modelled on `selkies-project/selkies` main `src/selkies/input_handler.py`. README in `scripts/gamepad-evdev-fallback/` documents when/how to use it.
>
> **NEXT: rebuild + push `:SteamUbuntu24.04VM`, fresh-boot, and test with a REAL controller in the browser** — connect a gamepad, press a button (the Selkies web client sends a gamepad-connect WebRTC message → Selkies creates `/tmp/selkies_js0.sock` → Steam's SDL3 classic opens `/dev/input/js0` → interposer → socket), and confirm Steam Big Picture detects the "Selkies Controller" + navigates + a game plays. Check `docker exec dpad-0 tail /tmp/selkies_js.log` (interposer log: "Intercepted open call for /dev/input/js0" + "Read config from socket" + js_event flow) and `docker exec dpad-0 ls /tmp/selkies_js*.sock` (socket appears when the browser connects a gamepad). If Steam doesn't detect via classic, switch to the evdev fallback (`scripts/gamepad-evdev-fallback/README.md`).
>
> **Multi-tenant (N-on-N) note:** the gamepad path is all per-container LD_PRELOAD + per-container `/dev/input/jsN` nodes — no shared state, no uinput/caps beyond what we have. One CDI container per GPU in `DPAD_GAMESCOPE` mode → N independent streamed+interactive+gamepad Steam UIs on one VM, no modesetting contention. The 2-GPU N-on-N test is the remaining headline payoff after gamepad is confirmed.
...
> 4. **Selkies' XFIXES cursor monitor crashed** (`Xlib.error.ConnectionClosedError: Display connection closed by server` in `start_cursor_monitor`) when the bridge was broken, which aborted the WebRTC session and looped the browser on `SESSION`. Root cause was python-xlib's `add_extension_event` TypeError during `display.Display()` (xfixes.init). Fix: `dpad_input_patch.py`'s `.pth` monkey-patches `Xlib.protocol.display.Display.add_extension_event` to promote a TYPE to a dict (the dispatcher already handles dicts), applied BEFORE any `Display()`. After the fix `xfixes.init` succeeds (`Found XFIXES version 4.0`). The gamescope Selkies launch now uses `--enable_cursors=true` (REQUIRED: gamescope headless does NOT composite the X cursor into its PipeWire output, so the XFIXES CSS-cursor overlay is the cursor's only visible source — see the 2026-07-07 CURSOR FIX update at the top).
