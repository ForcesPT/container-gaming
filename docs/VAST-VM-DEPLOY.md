...
| Mild flicker on Steam menu open/close | gamescope headless frame-pacing artifact on heavy UI transitions (`vblank: Using timerfd` synthetic 60Hz; not encode/network — independent of bitrate/fps, no PLI/FIR keyframe requests). No clean lever from our side; self-recovers. Not worth chasing. |
| Steam shows "no internet" / "can't reach Steam servers" icon but login+install+play work | Cosmetic. `api/store/login/community.steampowered.com` all return 200/302/404 from inside the container; only the non-existent `cm1.steampowered.com` hostname fails (Steam uses CM IPs, not that name). Steam network is functionally fine. Ignore. |
