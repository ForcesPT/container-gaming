...
| Mild flicker on Steam menu open/close | Root cause (confirmed in gamescope-steam.log): `pipewire: out of buffers` + `xwm: got the same buffer committed twice, ignoring` on the transition burst — gamescope headless's small PipeWire producer pool can't feed pipewiresrc's `always-copy=True` fast enough when the CEF menu window maps/recomposes, so frames are dropped/duplicated. Not encode/network (independent of bitrate/fps; no PLI/FIR). `-r 60` (explicit gamescope framerate) did NOT help (reverted) — the rate isn't the issue. The deeper fix (`always-copy=False` / larger pool) risks the `Already had a buffer` stream crash that `always-copy=True` was added to prevent → known mild limitation, not chased. |
