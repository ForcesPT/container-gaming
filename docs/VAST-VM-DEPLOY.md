...
> **`:dpad-SteamOS`** image (Vast KVM VM → Steam/gamescope → Selkies browser
> stream), from provisioning to a live, interactive browser stream. Every flag,
> env var, and gotcha below was validated on a Vast RTX 3060 / driver 580, 1-GPU
> VM. This is the authoritative "Vast.ai → VM → Docker → flags → everything" ref.
>
> This is the **Vast VM / full-Steam** image. The other image in this repo is
> **`:dpad-heroic`** (Vast Docker: Heroic desktop + Selkies, no Steam) — see
> `README.md`. Both share one Dockerfile (multi-stage) and use Selkies as the
> ONLY browser stream (mws, Sunshine, and Tailscale/native-Moonlight were removed).
>
> Companion docs: `PROJECT_STATE.md` (project state/continuation), `README.md`
> (deploy + the two-image model), `RUNPOD.md` (RunPod specifics).
...
- RTX 50/Blackwell (sm_120/sm_121) needs the **`-rtx50` image variant**
  (`:dpad-SteamOS-rtx50`, CUDA 12.8.1) — driver ≥570, in the #1249 range
  ...
  `docker build --target vast-vm --build-arg CUDA_VERSION=12.8.1 --build-arg CUDA_PKG=12-8 -t forcespt/dpadcloud-gaming:dpad-SteamOS-rtx50 .`
...
| `--device /dev/uinput` | (historically for Sunshine virtual input; Sunshine is removed) — the gamescope path uses XTest on `:0` so it doesn't need uinput. Harmless to keep; safe to drop on the gamescope-only path. |
...
| `DPAD_SELKIES_BIND` | `127.0.0.1` | Selkies signaling bind. `127.0.0.1` for the Vast-SSH-tunnel path (stream-bridge autossh reaches it on loopback); **`0.0.0.0` for UpCloud/direct-IP providers** so the browser + Caddy reverse-proxy reach Selkies at `<public-ip>:16100` direct (no autossh — the dpadplay VPS Caddy reverse-proxies HTTP straight to the container). See `cloud/docs/STATUS.md` §21. |
| `DPAD_AUDIO_PACKETLOSS` | `0` | Opus inband FEC % (e.g. `10`). Enables `opusenc inband-fec=true` + `packet-loss-percentage=N` + auto-bumps `fec_audio_bitrate = audio_bitrate * (1 + %/100)`. 0 = off. Browser decodes the FEC automatically. Robustness under audio packet loss. |
| `DPAD_VIDEO_PACKETLOSS` | `0` | ULP_RED video FEC % (e.g. `10`). Enables `transceiver fec-type=ULP_RED, fec-percentage=N` (RFC 2198 forward redundancy — the Boosteroid-style path, no round-trip needed, works under sustained loss). Auto-trims `fec_video_bitrate = video_bitrate / (1 + %/100)` to make room for the redundancy. 0 = off. NACK rtx retransmission (`rtxSsrc`) is ALSO active by default. |
...
| `DPAD_PROVIDER` | `vast` | Historically `runpod` armed the **dual-ICE TURN** config; that now fires automatically on ANY provider with a real public IP, so this is optional. vm-bootstrap still sets `runpod` (harmless). |
...
| `SUNSHINE_PASSWORD` | (unused) | Kept for back-compat only — Sunshine was removed (Selkies is the only stream). |
...
| `DPAD_IMAGE_TAG` | (auto) | Force a specific image tag (e.g. `:dpad-SteamOS-rtx50`), bypassing the `compute_cap ≥ 12 → -rtx50` auto-select. |
...
1. **NVENC #1249 (the blocker).** On driver 570+ a gamescope container pinned
   ...
   selects `:dpad-SteamOS-rtx50` on `compute_cap ≥ 12`. See §10.
...
| Symptom | Cause / Fix |
|---|---|
| Boot log `OpenGL renderer string: zink Vulkan 1.4(...)` (NOT `NVIDIA GeForce RTX ...`) + `/tmp/xorg.log` has `(II) GLX: Another vendor is already registered for screen 0` + the Selkies stream never comes up (browser `SESSION`-loops, no SDP offer) | The nvidia GLX vendor lost screen 0 to Mesa. nvidia 610.x ships only `libglxserver_nvidia.so` (no standalone `libglx.so`), so Mesa's `libglx-mesa0` `libglx.so` self-registers its swrast GLX vendor for screen 0 first → nvidia rejected → GL→Mesa/**zink** → `ximagesrc` can't capture → no stream. Worst on `cuda_max_good>=13.3` Blackwell hosts. **Fix (baked in):** the Dockerfile extracts `xserver-xorg-core`'s GLVND-neutral `libglx.so` into `/usr/lib/xorg/modules/nvidia/extensions/` (nvidia ModulePath, first) and the entrypoint hides Mesa's `libglx.so` on the nvidia-DDX path → Xorg loads the neutral dispatcher → only nvidia's vendor registers → nvidia wins screen 0. Boot log should say `nvidia GLX dispatcher present -> hiding Mesa's libglx.so` and the renderer should be `NVIDIA GeForce RTX ...`. If you see `WARNING: .../nvidia/extensions/libglx.so missing` the image is stale (rebuild — the Dockerfile GLVND bake failed). The `Another vendor already registered` line alone is cosmetic (nvidia can still win GL via direct DRI3); the real failure sign is the **zink renderer** + no stream. |
...
| Selkies "Connection failed" + browser gets only `host`/mDNS ICE candidates (no `typ relay`), but the in-container peer DOES relay | **The Vast host is not forwarding the mapped coturn port to the internet.** Some Vast hosts (e.g. behind a NAT) don't expose inbound mapped ports; the in-container peer still relays because it reaches coturn locally at `127.0.0.1:3478`, but the browser can't reach coturn externally → no browser relay → "Connection failed". **Not an image bug.** Diagnose: from outside, send a STUN/TURN Allocate (type `0x0003`) UDP packet to `<publicIp>:<VAST_UDP_PORT_3478>` — coturn must answer (a `401`). No reply ⇒ the host drops inbound; destroy + relaunch on a different host. Validated reachable: RTX 4070 / RTX 3060 single-GPU hosts. (Python probe: `py -3 -c "import socket,os,struct;s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM);s.settimeout(5);s.sendto(struct.pack('!HHI12s',0x0003,0,0x2112A442,os.urandom(12)),('<publicIp>',<VAST_UDP_PORT_3478>));d,_=s.recvfrom(2048);print('REACHABLE',len(d))" 2>&1 || echo UNREACHABLE`.) |
...
## 11. The two product paths (image split)

This repo builds **two images** from one multi-stage Dockerfile (the two
> use-cases have near-zero heavy overlap, so they ship separately instead of
> one 16 GB image carrying both). Both use Selkies as the only browser stream.

| Image | Provider / mode | Steam UI | Multi-tenant |
|---|---|---|---|
| **`:dpad-SteamOS`** | **Vast KVM VM** (`ubuntu_cli_22.04-2025-11-21`) + nested Docker, `DPAD_GAMESCOPE=1` (gamescope headless) | ✅ full interactive (video+audio+kbd+mouse+gamepad in browser), cloud/online | ✅ N-on-N-GPUs (no DRM master) — **validated 2-GPU (2× RTX 5060 Ti)** + CPU/RAM partitioning |
| `:dpad-SteamOS`, DFP path (`DPAD_GAMESCOPE=0`) | Vast KVM VM, Xorg + nvidia DDX, DRM master | ✅ full Steam | ❌ 1 full-Steam per VM (modeset singleton) |
| **`:dpad-heroic`** | **Vast Docker** (no userns), Heroic desktop | ❌ Steam blocked (no userns → CEF crashes); Heroic = Epic+GOG+Amazon + a cloud desktop (XFCE+Firefox) | single-user desktop |
| Bare-metal + QEMU/KVM/VFIO (one VM/GPU) | DFP | ✅ full Steam | ✅ N full-Steam on one host (separate driver instances) |
| RunPod Secure Cloud (userns) | use `:dpad-SteamOS` (DFP/gamescope) | likely ✅ (userns) | TBD |
| RunPod Community Cloud (no userns) | use `:dpad-heroic` | ❌ (no userns) | single-user |

The **gamescope headless path on the Vast KVM VM (`:dpad-SteamOS`) is the
validated MVP** and the path to N-on-N-GPUs multi-tenant full-Steam on one VM.